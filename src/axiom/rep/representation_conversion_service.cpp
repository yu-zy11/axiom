...
Result<MeshId> RepresentationConversionService::brep_to_mesh_shell(BodyId body_id, ShellId shell_id,
    ...
    if (!detail::has_body(*state_, body_id)) {
        return detail::invalid_input_result<MeshId>(
            *state_, diag_codes::kCoreInvalidHandle,
            "壳级BRep转网格失败：目标体不存在", "壳级BRep转网格失败");
    }
    if (!detail::has_shell(*state_, shell_id)) {
        return detail::invalid_input_result<MeshId>(
            *state_, diag_codes::kCoreInvalidHandle,
            "壳级BRep转网格失败：目标壳不存在", "壳级BRep转网格失败");
    }
    if (!detail::has_valid_tessellation_options(options)) {
        return detail::invalid_input_result<MeshId>(
            *state_, diag_codes::kCoreParameterOutOfRange,
            "壳级BRep转网格失败：三角化误差参数必须为正数", "壳级BRep转网格失败");
    }
    const auto body_it = state_->bodies.find(body_id.value);
    const auto& body = body_it->second;
    ...
    if (!shell_owned) {
        return detail::invalid_input_result<MeshId>(
            *state_, diag_codes::kCoreParameterOutOfRange,
            "壳级BRep转网格失败：壳不属于该体的 owned 壳集合", "壳级BRep转网格失败");
    }
    const auto shell_it = state_->shells.find(shell_id.value);
    if (shell_it == state_->shells.end() || shell_it->second.faces.empty()) {
        return detail::invalid_input_result<MeshId>(
            *state_, diag_codes::kCoreParameterOutOfRange,
