...
/// SemVer 2.0 风格：仅取 **核心版本**（首个 `-` 预发布或 `+` 构建元数据之前），供 SameMajor/SameMinor 与宿主三元组对齐。
/// `Exact` 模式不使用此剥离（见 `plugin_api_version_declared_compatible`）。
std::string_view semver_core_version_prefix(std::string_view s) {
    const auto dash = s.find('-');
    const auto plus = s.find('+');
    auto end = s.size();
    if (dash != std::string_view::npos) {
        end = std::min(end, dash);
    }
    if (plus != std::string_view::npos) {
        end = std::min(end, plus);
    }
    return trim_manifest_field(s.substr(0, end));
}

bool try_parse_plugin_api_version(std::string_view s, ParsedPluginApiVersion& out, bool semver_core_only) {
    ...
    if (semver_core_only) {
        s = semver_core_version_prefix(s);
        if (s.empty()) {
            return false;
        }
    }
...
bool plugin_api_version_declared_compatible(std::string_view declared_trimmed,
    ...
    // 宿主版本串通常无预发布后缀；兼容模式下仍剥离 +build，避免 `1.0+ci` 一类声明无法解析。
    if (!try_parse_plugin_api_version(expected_sdk_api, host_p, true) ||
        !try_parse_plugin_api_version(declared_trimmed, decl_p, true)) {
        return false;
    }
...
Result<CurveId> PluginRegistry::invoke_registered_curve(std::string_view implementation_type_name, PluginCurveDesc desc) {
    const auto key = trim_manifest_field(implementation_type_name);
    if (key.empty()) {
        return error_result<CurveId>(StatusCode::InvalidInput, {},
                                     {make_warn(diag_codes::kPluginCapabilityIncomplete,
                                                "调用曲线插件时 implementation_type_name 为空或仅空白")});
    }
    const auto desc_tn = trim_manifest_field(desc.type_name);
    if (desc_tn.empty()) {
        desc.type_name = std::string(key);
    } else if (desc_tn != key) {
        return error_result<CurveId>(
            StatusCode::InvalidInput, {},
            {make_warn(diag_codes::kPluginCapabilityIncomplete,
                       "PluginCurveDesc::type_name 与 implementation_type_name 不一致（去空白后须相同）")});
    }
    ...
    return error_result<CurveId>(StatusCode::InvalidInput, {},
                                 {make_warn(diag_codes::kPluginNotRegistered, "未找到该 implementation_type_name 的曲线插件")});
}
...
Result<BodyId> PluginRegistry::invoke_registered_importer(std::string_view implementation_type_name, std::string_view path) {
    ...
    if (key.empty()) {
        return error_result<BodyId>(StatusCode::InvalidInput, {},
                                    {make_warn(diag_codes::kPluginCapabilityIncomplete,
                                               "调用导入插件时 implementation_type_name 为空或仅空白")});
    }
    ...
    return error_result<BodyId>(StatusCode::InvalidInput, {},
                                {make_warn(diag_codes::kPluginNotRegistered, "未找到该 implementation_type_name 的导入插件")});
}
...
Result<void> PluginRegistry::invoke_registered_exporter(std::string_view implementation_type_name, BodyId body_id,
    ...
    if (key.empty()) {
        return error_void(StatusCode::InvalidInput, {},
                          {make_warn(diag_codes::kPluginCapabilityIncomplete,
                                     "调用导出插件时 implementation_type_name 为空或仅空白")});
    }
    ...
    return error_void(StatusCode::InvalidInput, {},
                      {make_warn(diag_codes::kPluginNotRegistered, "未找到该 implementation_type_name 的导出插件")});
}
...
Result<OpReport> PluginRegistry::invoke_registered_repair(std::string_view implementation_type_name, BodyId body_id,
    ...
    if (key.empty()) {
        return error_result<OpReport>(StatusCode::InvalidInput, {},
                                      {make_warn(diag_codes::kPluginCapabilityIncomplete,
                                                 "调用修复插件时 implementation_type_name 为空或仅空白")});
    }
    ...
    return error_result<OpReport>(StatusCode::InvalidInput, {},
                                  {make_warn(diag_codes::kPluginNotRegistered, "未找到该 implementation_type_name 的修复插件")});
}
