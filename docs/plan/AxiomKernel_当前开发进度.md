...
- **曲线类型**
  - **已完成（基础可用）**：直线、圆、椭圆、Bezier、BSpline（近似）、NURBS（近似）
  - **未开始/缺失**：线段、抛物线、双曲线、复合曲线、`PCurve`
- **曲面类型**
  - **已完成（基础可用）**：平面、圆柱、圆锥、球面、环面、BSpline（控制网格）、NURBS（控制网格）
  - **未开始/缺失**：旋转面、扫掠面、Bezier 曲面、修剪面、偏置面（几何层意义上的 surface offset）
- **求值/查询能力**
...
- **部分完成**：STEP 导入导出主链路雏形、导入后自动验证与诊断回传
- **未开始/缺失**：IGES/BREP/STL 完整互操作；第二阶段 OBJ/3MF/glTF；导出严格/兼容模式与“输出验证报告”闭环

### 7.10 三角化与显示支撑（Rep）

- **部分完成**：三角化语义与参数校验、基础检查报告雏形
- **未开始/缺失**：曲率敏感细分、局部重三角化、显示缓存、纹理参数等完整输出
