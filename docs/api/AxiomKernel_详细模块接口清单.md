...
`SurfaceFactory::make_bspline/make_nurbs` 对 u/v 两轴的显式结点采用相同的有限、非减、非零有效域及 `degree + 1` 重数上限；任一轴不满足时返回 `InvalidInput` 和 `AXM-GEO-E-0002`，且不创建曲面或改动求值缓存。合法满重数断点保留，并按所在非空单侧曲面片求值。
