...
- **数据交换**：`STEP`/`IGES` 等为序列化介质；导入后权威表示为 Axiom 内部 `Body`/拓扑/几何句柄。可选外部解析库仅能通过桥接层接入，且须符合许可证与模块边界（详见 **[模块依赖图与时序图](AxiomKernel_模块依赖图与时序图.md)**；新增外部依赖时必须在 [ADR 索引](../decisions/README.md) 登记决策）。
