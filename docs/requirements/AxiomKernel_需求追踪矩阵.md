...
| FR-DIAG-001 | 结构化诊断与导出 | Diagnostics, all | 横向 | 受限可用 | `axiom_diagnostics_test`（批量 JSON 完整证据、转义、无效 ID 拒绝及文件/源报告不污染）、`axiom_boolean_prep_test`（布尔预处理告警阶段/实体/JSON、退化与前置失败不污染） | 全部重量级流程的阶段、实体和数值证据 |
