...
|---|---|---|---|---|---|---|
| FR-GEO-001 | 几何对象、求值与变换 | Math, Geo | Stage 2 | 受限可用 | `axiom_math_services_test`, `axiom_geometry_test` | 已补显式样条结点逆序/零长度有效域/重数超过 degree + 1 拒绝与失败不污染回归，以及合法高重数断点单侧求值回归；仍需扩展曲线/曲面类型及精度语义 |
