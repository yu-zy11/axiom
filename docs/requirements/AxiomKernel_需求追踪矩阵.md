...
| FR-DIAG-001 | 结构化诊断与导出 | Diagnostics, all | 横向 | 受限可用 | `axiom_diagnostics_test`（批量 JSON 完整证据、转义；第 14 切片批量文本完整问题证据；第 24 切片阶段聚合导出的正常/空阶段、参数/打开/设备写入失败、文件/源报告不污染与重试）、`axiom_boolean_prep_test`（布尔预处理告警阶段/实体/JSON、退化与前置失败不污染）、`axiom_io_workflow_test`（严格网格导出 QA 的阶段、Body、退化与失败不污染） | 全部重量级流程的阶段、实体和数值证据 |
