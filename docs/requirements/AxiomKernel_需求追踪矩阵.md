...
|---|---|---|---|---|---|---|
| FR-GEO-001 | 几何对象、求值与变换 | Math, Geo | Stage 2 | 受限可用 | `axiom_math_services_test`, `axiom_geometry_test` | 已补曲线及曲面双轴显式样条结点校验、高重数断点及非夹持重复端点单侧求值；第 16/26 切片分别使曲线非空结点分段、曲面非空张量积结点片参与最近点初值搜索；第 21 切片拒绝曲面零有效域与超限重数并保证失败不污染；第 31 切片固定曲面满重数断点的一二阶偏导与曲率单侧语义；仍需扩展曲线/曲面类型及全局精度语义 |
