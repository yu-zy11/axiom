...
| NFR-DIA-001 | 可解释失败 | 受限可用 | 稳定错误码、`diagnostic_id`、阶段和问题实体 | diagnostics 及 workflow 测试；`axiom_boolean_prep_test` 覆盖诊断开关两种模式下的早期失败阶段、输入实体、检索/JSON 与失败不污染；第 10 切片补齐非法 BooleanOp 拒绝及合法枚举回归，拒绝不触发 Eval 失效传播 | 支持范围内无静默失败 |
