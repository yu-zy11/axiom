...
| `AXM-IO-E-0004` | Error | 导入失败；STEP 早期失败绑定 `io.import.step.input/path/open`；OBJ 物化前失败绑定 `io.import.obj.input/path/open/parse`；STL 物化前失败绑定 `io.import.stl.input/path/open/read/parse/validation`。OBJ/STL 退化三角形复用 `AXM-VAL-E-0002` 与各自的 `.validation` 阶段。路径及解析阶段尚无模型实体可关联。 |
